...
public class LegoTransactionDemo {
	...
	public BasicAbiTransaction makeHelloWorldSetTest() throws NoSuchTransactionFileException {
		...
		abiTx.setContractName(contractName).setAbiContent(abiContent).setMethodName("set").setParams(params)
	...
	public BasicAbiTransaction makeHelloWorldDeployTest() throws NoSuchTransactionFileException {
		...
		String contractName = "HelloWorld";
		String abiContent = lt.bcosClientWrapper.contractLoader.getABIByContractName(contractName);
		String contractBin = lt.bcosClientWrapper.contractLoader.getBinaryByContractName(contractName);
		List<Object> params = new ArrayList<Object>();
		abiTx.setContractName(contractName).setAbiContent(abiContent).setParams(params)
	...
	//同步部署演示
	public void testDeploy() throws NoSuchTransactionFileException, JsonProcessingException, ABICodecException, TransactionException, IOException
	{
		BasicAbiTransaction abiTx = makeHelloWorldDeployTest();
		ISignTransaction signTxImpl = new KeyToolSignTransaction(lt.bcosClientWrapper.getTxCryptoSuite());
	...
	//同步交易演示
	public TransactionResponse testTx() throws NoSuchTransactionFileException, JsonProcessingException, ABICodecException, TransactionException, IOException
	{
		//构建一个调用接口的测试交易
	...
	//异步交易演示
	public void testTxAsyn() throws NoSuchTransactionFileException, ABICodecException, JsonProcessingException,
	...
	class TestCallback extends TransactionCallback
	{
		...
		public void onResponse(TransactionReceipt receipt) {
			...
			TransactionDecoderInterface transactionDecoder = new TransactionDecoderService(lt.bcosClientWrapper.getClient().getCryptoSuite());
			TransactionResponse response;
			try {
				response = transactionDecoder.decodeReceiptWithValues(abiTx.abiContent, abiTx.methodName, receipt);
	...
	public CallResponse testCall() throws ABICodecException, TransactionBaseException {
		int chainId = 1;
		BasicAbiTransaction abiTx = new BasicAbiTransaction("HelloWorld","","get",sampleContractAddress,new ArrayList<>());
		CallResponse callResponse = lt.bcosClientWrapper.transactionProcessor.sendCallByContractLoader(
				abiTx.getContractName(), abiTx.getTo(), abiTx.getMethodName(), abiTx.getParams());
