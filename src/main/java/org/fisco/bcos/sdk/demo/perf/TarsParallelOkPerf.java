...
public class TarsParallelOkPerf {
	...
	public static void main(String[] args) throws ContractException, IOException, InterruptedException {
		try {
			...
			ThreadPoolService threadPoolService = new ThreadPoolService("ParallelOkPerf",
					Runtime.getRuntime().availableProcessors());

			if (perfType.compareToIgnoreCase("parallelok") == 0) {
				parallelOkPerf(groupId, command, count, qps, conflictPercent, threadPoolService, isParallel);
				...
				System.out.println("invalid perf option: " + perfType + ", only support parallelok/precompiled now");
				Usage();
			}
		} catch (Exception e) {
	...
	public static void parallelOkPerf(String groupId, String command, Integer count, Integer qps,
			Integer conflictPercent, ThreadPoolService threadPoolService, boolean isParallel)
			throws IOException, InterruptedException, ContractException {
		System.out.println("====== ParallelOk trans, count: " + count + ", qps:" + qps + ", groupId: " + groupId
				+ ", conflictPercent: " + conflictPercent + ", isParallel: " + isParallel);
		ParallelOk parallelOk;
		ParallelOkDemo parallelOkDemo;
		switch (command) {
		case "add":
			// deploy ParallelOk
			parallelOk = ParallelOk.deploy(client, client.getCryptoSuite().getCryptoKeyPair(), isParallel);
		...
		case "transfer":
			...
			parallelOk = ParallelOk.load(dagUserInfo.getContractAddr(), client,
					client.getCryptoSuite().getCryptoKeyPair());
			System.out.println("====== ParallelOk trans, load success, address: " + parallelOk.getContractAddress());
		...
		case "generate":
			...
			parallelOk = ParallelOk.load(dagUserInfo.getContractAddr(), client,
					client.getCryptoSuite().getCryptoKeyPair());
			parallelOkDemo = new ParallelOkDemo(parallelOk, dagUserInfo, threadPoolService);
			parallelOkDemo.generateTransferTxs(BigInteger.valueOf(count), "parallelOKTxs.txt", BigInteger.valueOf(qps),
	...
	public static void dagTransferPerf(String groupId, String command, Integer count, Integer qps,
			Integer conflictPercent, ThreadPoolService threadPoolService)
			throws IOException, InterruptedException, ContractException {
		System.out.println("====== DagTransfer trans, count: " + count + ", qps:" + qps + ", groupId: " + groupId
		...
		case "generate":
			dagUserInfo.loadDagTransferUser();
			dagPrecompiledDemo = new DagPrecompiledDemo(client, dagUserInfo, threadPoolService);
			dagPrecompiledDemo.generateTransferTxs(BigInteger.valueOf(count), "dagTxs.txt", BigInteger.valueOf(qps),
					BigInteger.valueOf(conflictPercent));
			break;
		default:
			System.out.println("invalid command: " + command);
