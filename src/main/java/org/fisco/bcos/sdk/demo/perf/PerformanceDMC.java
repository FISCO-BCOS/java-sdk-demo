...
public class PerformanceDMC {
	...
	public static void main(String[] args) throws ContractException, IOException, InterruptedException {
		try {
			...
			ThreadPoolService threadPoolService = new ThreadPoolService("DMCClient",
	...
	public static void start(String groupId, int userCount, int count, Integer qps, ThreadPoolService threadPoolService)
		...
		IntStream.range(0, count).parallel().forEach(i -> {
			...
			account.addBalance(BigInteger.valueOf(value), new TransactionCallback() {
				@Override
		...
		IntStream.range(0, summary.length).parallel().forEach(i -> {
			limiter.acquire();
			...
			try {
				...
				if (balance.longValue() != expectBalance) {
					System.out.println("Check failed! Account[" + i + "] balance: " + balance
							+ " not equal to expected: " + expectBalance);
				}
			} catch (ContractException e) {
				e.printStackTrace();
			}
