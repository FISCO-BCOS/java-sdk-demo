...
public class Collector {
	...
	public void onMessage(TransactionReceipt receipt, Long cost) {
		try {
			...
			if (!receipt.isStatusOK()) {
				logger.error("error receipt, status: {}, output: {}, message: {}", receipt.getStatus(),
						receipt.getOutput(), receipt.getMessage());
				errorMessage = true;
			}
			stat(errorMessage, cost);
		} catch (Exception e) {
	...
	public void onPrecompiledMessage(RetCode retCode, Long cost) {
		try {
			...
			if (retCode.getCode() != PrecompiledRetCode.CODE_SUCCESS.code) {
				logger.error("error retCode, code: {}, message: {}", retCode.code, retCode.getMessage());
				errorMessage = true;
			}
			stat(errorMessage, cost);
		} catch (Exception e) {
	...
	public void report() {
		...
		System.out.println("TPS(exclude error requests): " + (total - error.get()) / ((double) totalTime / 1000));
		System.out.println("Avg time cost: " + totalCost.get() / total + "ms");
		System.out.println("Errors: " + error.get());

		System.out.println("Time group:");
		System.out.println("0    < time <  50ms   : " + less50 + "  : " + (double) less50.get() / total * 100 + "%");
		System.out.println("50   < time <  100ms  : " + less100 + "  : " + (double) less100.get() / total * 100 + "%");
		System.out.println("100  < time <  200ms  : " + less200 + "  : " + (double) less200.get() / total * 100 + "%");
		System.out.println("200  < time <  400ms  : " + less400 + "  : " + (double) less400.get() / total * 100 + "%");
		System.out
				.println("400  < time <  1000ms : " + less1000 + "  : " + (double) less1000.get() / total * 100 + "%");
		System.out
				.println("1000 < time <  2000ms : " + less2000 + "  : " + (double) less2000.get() / total * 100 + "%");
		System.out.println(
				"2000 < time           : " + timeout2000 + "  : " + (double) timeout2000.get() / total * 100 + "%");
	}
